...
class DefaultController extends Controller
{
                ...
                if ($personal_accs) {
                    foreach ($personal_accs as $index => $personal_acc) {
                        $result['data']['bills'][$index] = [
                            ...
                            'bill' => $personal_accs->value,
                            'dept' => $personal_acc->dept
                        ];
                    }
                }
            } else {
                ...
                if (!empty($_POST['bill']) && !empty($_POST['service_id'])) {
                    ...
                    if ($service) {
                        ...
                        $history = PaymentData::find()
                            ->where('personal_acc_id = :personal_date_id', [':personal_date_id' => $_POST['bill']])
    ...
    public function actionGetHistoryForUser()
    {
        $token = $this->getToken();
        ...
        if ($token) {
            ...
            if (!empty($user)) {
                $personalAccs = PersonalAccount::find()
                    ->where('user_id = :user_id', [':user_id' => $user->id])
                    ->all();

                if ($personalAccs) {
                    $services = [];
                    foreach ($personalAccs as $personalAcc) {
                        $services[] = Services::find()
                            ->where('id = :id', [':id' => $personalAcc->service_id])
                            ->one();
                    }
